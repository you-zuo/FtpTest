...
import com.youzuo.ftptest.ftp.App;
...
public class SessionThread extends Thread {
    ...
    //private File workingDir = Environment.getRootDirectory();
    ...
    public boolean sendViaDataSocket(String string) {
        try {
            ...
            Log.d("Session Thread", "Using data connection encoding: " + encoding);
            return sendViaDataSocket(bytes, 0, bytes.length);
        } catch (UnsupportedEncodingException e) {
            Log.e("Session Thread", "Unsupported encoding for data socket send");
            return false;
        }
    }
    ...
    public boolean sendViaDataSocket(byte[] bytes, int start, int len) {
        ...
        if (dataOutputStream == null) {
            Log.i("Session Thread", "Can't send via null dataOutputStream");
            return false;
        }
        if (len == 0) {
        ...
        try {
            ...
            Log.e("Session Thread", "Couldn't write output stream for data socket, error:" + e.toString());
            return false;
        }
    ...
    public int receiveFromDataSocket(byte[] buf) {
        ...
        if (dataSocket == null) {
            Log.i("Session Thread", "Can't receive from null dataSocket");
            return -2;
        }
        if (!dataSocket.isConnected()) {
            Log.i("Session Thread", "Can't receive from unconnected socket");
            return -2;
        }
        ...
        try {
            ...
            Log.i("Session Thread", "Error reading data socket");
            return 0;
        }
        return bytesRead;
    }
    ...
    public boolean openDataSocket() {
        try {
            ...
            if (dataSocket == null) {
                Log.i("Session Thread", "dataSocketFactory.onTransfer() returned null");
                return false;
            }
            ...
            Log.i("Session Thread", "IOException getting OutputStream for data socket");
            dataSocket = null;
            return false;
        }
    }
    ...
    public void closeDataSocket() {
        Log.d("Session Thread", "Closing data socket");
    ...
    public void quit() {
        Log.d("Session Thread", "SessionThread told to quit");
        closeSocket();
    }

    @Override
    public void run() {
        Log.i("Session Thread", "SessionThread started");
        ...
        try {
            ...
            while (true) {
                ...
                if (line != null) {
                    Log.d("Session Thread", "Received line from client: " + line);
                    FtpCmd.dispatchCommand(this, line);
                } else {
                    Log.i("Session Thread", "readLine gave null, quitting");
                    break;
                }
            }
        } catch (IOException e) {
            Log.i("Session Thread", "Connection was dropped");
        }
        closeSocket();
    }
    ...
    public void writeBytes(byte[] bytes) {
        try {
            ...
            Log.i("Session Thread", "Exception writing socket");
            closeSocket();
        }
    }
    ...
    public void writeString(String str) {
        ...
        try {
            ...
            Log.e("Session Thread", "Unsupported encoding: " + encoding);
    ...
    public void authAttempt(boolean authenticated) {
        if (authenticated) {
            Log.i("Session Thread", "Authentication complete");
            userAuthenticated = true;
        } else {
            authFails++;
            Log.i("Session Thread", "Auth failed: " + authFails + "/" + MAX_AUTH_FAILS);
            if (authFails > MAX_AUTH_FAILS) {
                Log.i("Session Thread", "Too many auth fails, quitting session");
                quit();
            }
        }
    }
    ...
    public void setWorkingDir(File workingDir) {
        try {
            ...
            Log.i("Session Thread", "SessionThread canonical error");
        }
    }
