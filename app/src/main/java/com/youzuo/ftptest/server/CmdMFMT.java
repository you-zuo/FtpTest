...
public class CmdMFMT extends FtpCmd implements Runnable {
    ...
    @Override
    public void run() {
        Log.d("CmdMFMT","run: MFMT executing, input: " + mInput);
        ...
        if (splitPosition == -1) {
            ...
            Log.d("CmdMFMT","run: MFMT failed, wrong number of parameters");
            return;
        }
        ...
        try {
            ...
            Log.d("CmdMFMT","run: MFMT failed, unable to parse parameter time-val");
            return;
        }
        ...
        if (!file.exists()) {
            ...
            Log.d("CmdMFMT","run: MFMT failed, file does not exist");
            return;
        }
        ...
        if (!success) {
            ...
            Log.d("CmdMFMT","run: MFMT failed, unable to modify last modification time");
        ...
        Log.d("CmdMFMT","run: MFMT completed successful");
    }
