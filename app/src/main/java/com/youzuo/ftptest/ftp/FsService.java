...
import com.youzuo.ftptest.R;
import com.youzuo.ftptest.server.SessionThread;
import com.youzuo.ftptest.server.TcpListener;
...
public class FsService extends Service implements Runnable {
    ...
    static public final String ACTION_REQUEST_START = "in.naveen.dotftp.REQUEST_START";
    static public final String ACTION_REQUEST_STOP = "in.naveen.dotftp.REQUEST_STOP";

    // Service will (global) broadcast when server start/stop
    static public final String ACTION_STARTED = "in.naveen.dotftp.FTPSERVER_STARTED";
    static public final String ACTION_STOPPED = "in.naveen.dotftp.FTPSERVER_STOPPED";
    static public final String ACTION_FAILEDTOSTART = "in.naveen.dotftp.FTPSERVER_FAILEDTOSTART";
    ...
    private Notification buildForegroundNotification() {
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(this, channelId)
                ...
                .setContentTitle("FTP")
                .setContentText("Background FTP Server")
    ...
    private static void warnIfNoExternalStorage() {
        ...
        if (!storageState.equals(Environment.MEDIA_MOUNTED)) {
            ...
            Toast.makeText(App.getAppContext(), "没有外置储存", Toast.LENGTH_LONG).show();
        }
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null && intent.getAction() != null) {
            Log.d("FsService", "onStartCommand called with action: " + intent.getAction());
        ...
        serverThread.start();
        Log.d(TAG, "Created server thread");
    ...
    public void run() {
        ...
        Log.i(TAG, "Checking if i should exit...." + shouldExit);
        while (!shouldExit) {
            ...
            try {
                if (wifiListener == null) {
                    ...
                    // so spawn it
                    Log.i(TAG, "Starting TCP listener");
                    wifiListener = new TcpListener(listenSocket, this);
                    wifiListener.start();
                    Log.i(TAG, "Started TCP listener");
                }
                ...
                for (InetAddress address : Collections.list(networkInterface.getInetAddresses())) {
                    if (!address.isLoopbackAddress()
                        ...
                        if (returnAddress != null) {
                            Log.w("FsService", "Found more than one valid address local inet address, why???");
                        }
